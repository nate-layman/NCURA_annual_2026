...
(function () {
  ...
  /* --- AI spectrum (interpretability-flexibility slider) --- */
  document.querySelectorAll('.ai-spectrum').forEach(function (root) {
    var slider = root.querySelector('.ai-spectrum-range');
    if (!slider) return;
    var raw = root.getAttribute('data-stops');
    if (!raw) return;
    var stops;
    try { stops = JSON.parse(raw); } catch (e) { return; }
    ...
    var stopsRow = root.querySelector('.ai-spectrum-stops');
    if (stopsRow && !stopsRow.children.length) {
      stops.forEach(function (s, i) {
        var span = document.createElement('span');
        span.className = 'ai-spectrum-stop';
        span.dataset.idx = i;
    ...
    var nameEl = root.querySelector('.ai-spectrum-name');
    var descEl = root.querySelector('.ai-spectrum-desc');
    var interpFill = root.querySelector('.ai-spectrum-metric-fill[data-metric="interpretability"]');
    var flexFill = root.querySelector('.ai-spectrum-metric-fill[data-metric="flexibility"]');
    var stopEls = root.querySelectorAll('.ai-spectrum-stop');
    ...
    function update() {
      ...
      if (interpFill) interpFill.style.width = s.interpretability + '%';
      if (flexFill) flexFill.style.width = s.flexibility + '%';
